...
export default function animateHeroTexts({ leftEl, rightEl, planeEl, togetherEl, tejasEl, descriptionEl, threePlaneApi, onRightShown, heroSectionEl } = {}) {
  ...
  function createPlaneScroll() {
    ...
    function pushProxy() {
      try {
        if (threePlaneApi && typeof threePlaneApi.setTargetPosition === 'function') {/* Lines 76-77 omitted */}
        if (threePlaneApi && typeof threePlaneApi.setTargetRotation === 'function') {/* Lines 79-80 omitted */}
      } catch (e) {
        // ignore
      }
    }
    ...
    STAGES.slice(0, 2).forEach((s) => {
      if (Array.isArray(s.keys) && s.keys.length) {
        s.keys.forEach((k) => {/* Lines 168-179 omitted */})
      } else {
        planeTL.to(proxy, {
          px: s.pos.x,
          py: s.pos.y,
    ...
    if (mvvgEl && Array.isArray(STAGE4_KEYS) && STAGE4_KEYS.length) {
      ...
      const stage4TL = gsap.timeline({
        scrollTrigger: {
          ...
          onToggle: (self) => {
            try {/* Lines 219-229 omitted */} catch (e) {}
          }
        }
      })
      ...
      const sponsorEl = (typeof document !== 'undefined') ? (() => {
        ...
        if (mainCta) {
          const s = mainCta.closest('section') || null
          if (s) return s
        }
        ...
        if (cta) {
          const s = cta.closest('section') || null
          if (s) return s
        }
      ...
      if (sponsorEl && Array.isArray(STAGE6_KEYS) && STAGE6_KEYS.length) {
        ...
        const stage6TL = gsap.timeline({
          scrollTrigger: {
            ...
            markers: false,
            invalidateOnRefresh: true,
            onToggle: (self) => {/* Lines 280-293 omitted */}
          }
        })

        // Play Stage 5 first when sponsor section is reached
        if (Array.isArray(STAGE5_KEYS) && STAGE5_KEYS.length) {/* Lines 299-312 omitted */}

        // Then play Stage 6
        STAGE6_KEYS.forEach((k) => {/* Lines 316-327 omitted */})
      ...
      if (testimonialsEl && Array.isArray(STAGE7_KEYS) && STAGE7_KEYS.length) {
        const stage7TL = gsap.timeline({
          scrollTrigger: {
            ...
            markers: false,
            invalidateOnRefresh: true,
            onToggle: (self) => {/* Lines 350-363 omitted */}
          }
        })

        STAGE7_KEYS.forEach((k) => {/* Lines 368-379 omitted */})
      }
    }
    ...
    // Prefer the SECTION element that contains the about content so
    // we don't animate an inner div while the parent section remains hidden.
    let aboutEl = null
    if (typeof document !== 'undefined') {
      const candidate = document.querySelector('#about, [data-about], .about-section, .about, section[aria-labelledby="about-heading"]')
      if (candidate) {
        // If the found element isn't a <section>, try to use its closest section parent.
        if (candidate.tagName && candidate.tagName.toLowerCase() !== 'section') {
          const parentSection = candidate.closest('section')
          aboutEl = parentSection || candidate
        } else {
          aboutEl = candidate
        }
      }
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          gsap.to(aboutEl, {
            ...
            duration: 0.8,
            ease: 'power2.out',
            onComplete: () => {/* Lines 420-425 omitted */}
          })
        },
        onLeaveBack: () => {
          gsap.to(aboutEl, {
            ...
            duration: 0.5,
            ease: 'power2.in',
            onComplete: () => {/* Lines 434-437 omitted */}
          })
        },
        markers: false,
        invalidateOnRefresh: true
      ...
      if (aboutEl && Array.isArray(STAGES) && STAGES[2] && Array.isArray(STAGES[2].keys)) {
        const aboutTL = gsap.timeline({
          scrollTrigger: {
            ...
            markers: false,
            invalidateOnRefresh: true,
            onToggle: (self) => {/* Lines 459-471 omitted */}
          }
        })

        STAGES[2].keys.forEach((k) => {/* Lines 476-487 omitted */})
      }
    } catch (e) {
      // non-fatal
    }
  ...
  try {
    ...
    if (mvvgEl) {
      gsap.set(mvvgEl, { opacity: 0, y: 50 })
      gsap.to(mvvgEl, {
        opacity: 1,
        y: 0,
        duration: 1,
    ...
    // About reveal: create independently so it runs even if the plane
    // scroll timeline hasn't been initialized yet.
    const aboutCandidate = (typeof document !== 'undefined') ? document.querySelector('#about, [data-about], .about-section, .about, section[aria-labelledby="about-heading"]') : null
    let aboutSectionEl = null
    if (aboutCandidate) {
      aboutSectionEl = aboutCandidate.tagName && aboutCandidate.tagName.toLowerCase() === 'section' ? aboutCandidate : (aboutCandidate.closest ? aboutCandidate.closest('section') : aboutCandidate)
    }

    if (aboutSectionEl) {
      try {
        gsap.set(aboutSectionEl, { opacity: 0, y: 30 })
        ScrollTrigger.create({
          ...
          invalidateOnRefresh: true,
          onEnter: () => {
            gsap.to(aboutSectionEl, { opacity: 1, y: 0, duration: 0.8, ease: 'power2.out' })
          },
          onLeaveBack: () => {
            gsap.to(aboutSectionEl, { opacity: 0, y: 30, duration: 0.5, ease: 'power2.in' })
          }
        })

        // If the About section is already visible, make it visible immediately
        if (typeof window !== 'undefined') {
          const rect = aboutSectionEl.getBoundingClientRect()
          if (rect.top < window.innerHeight * 0.75 && rect.bottom > 0) {
            gsap.to(aboutSectionEl, { opacity: 1, y: 0, duration: 0.1 })
          }
        }
      } catch (e) {}
    }

    // Sponsor reveal handled inside Home component to avoid double-init and flash
    ...
    if (testimonialsHome) {
      ...
      if (tItems && tItems.length) {
        gsap.set(tItems, { opacity: 0, scale: 0.96, y: 10 })
        gsap.to(tItems, {
          opacity: 1,
          scale: 1,
          y: 0,
...
export function animateTypewriter({ el, fullText = '', wordDelay = 60, startDelay = 50, cursorColor = '#75fbfd' } = {}) {
  ...
  tokens.forEach((token, i) => {
    ...
    const t = setTimeout(() => {
      // If this token is our newline marker, insert a <br> and start a new text node
      if (token === marker) {
        const br = document.createElement('br')
        el.insertBefore(br, cursor)
        // start a fresh text node after the <br>
        const newText = document.createTextNode('')
        el.insertBefore(newText, cursor)
        // store reference to the latest text node on the element for subsequent tokens
        el._currentTextNode = newText
      } else {
        // Ensure we have a current text node to append into
        if (!el._currentTextNode) {
          const node = document.createTextNode('')
          el.insertBefore(node, cursor)
          el._currentTextNode = node
        }
        const node = el._currentTextNode
        // Add a leading space if the node already has content
        const prefix = node.data && node.data.length > 0 ? ' ' : ''
        node.appendData(prefix + token)
      }
    }, delayAt)
    timers.push(t)
  })
